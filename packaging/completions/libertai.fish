...
complete -c libertai -n "__fish_libertai_using_subcommand code" -l acp -d 'Speak the Agent Client Protocol (ACP) on stdin/stdout instead of starting a terminal session, so an editor can drive the agent. ACP is Zed\'s editor↔agent standard (also supported by the JetBrains IDEs); it is line-delimited JSON-RPC 2.0, so stdout carries protocol frames and nothing else — logs and warnings go to stderr. Not for interactive use: point your editor at `libertai code --acp`. See the README for Zed and JetBrains setup'
