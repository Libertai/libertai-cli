...
fn format_mcp_doctor_summary(cfg: &LibertaiConfig) -> String {
    ...
    format!(
        "{} configured; mcp_call {}, {} named tool(s), resource reader {}, prompt getter {}, {} subscription candidate(s); stdio/http reuse on",
        cfg.mcp_servers.len(),
...
fn print_mcp_status(command: McpCommand) {
    ...
    match command {
        ...
        McpCommand::Reset => {
            ...
            println!(
                "{DIM}  /mcp reset:{RESET} closed {closed} terminal stdio/HTTP MCP session{}.",
                if closed == 1 { "" } else { "s" }
            );
            println!("{DIM}  note:{RESET} SSE MCP calls are still per-call; stdio and Streamable HTTP MCP tools/resources/prompts reuse live CLI sessions until reset or process exit.");
        }
        McpCommand::Open => {
            println!(
...
mod tests {
    ...
    fn doctor_mcp_summary_reports_cli_exposure() {
        ...
        assert_eq!(
            format_mcp_doctor_summary(&cfg),
            "1 configured; mcp_call on, 1 named tool(s), resource reader on, prompt getter on, 1 subscription candidate(s); stdio/http reuse on"
        );
        assert_eq!(
            format_mcp_doctor_summary(&LibertaiConfig::default()),
            "0 configured; mcp_call off, 0 named tool(s), resource reader off, prompt getter off, 0 subscription candidate(s); stdio/http reuse on"
        );
    }
