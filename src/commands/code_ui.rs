...
//! background-agent management functions, and the slash-command parsing
//! functions (preserved for future TUI slash-command dispatch).
//!
//! Many functions here are currently dead code — they were called by
//! the old `repl_loop` which has been removed. They are kept so the
//! TUI can re-wire them without re-implementing the parsing logic.
...
use pi::sdk::{AgentEvent, AgentSessionHandle, ThinkingLevel};
...
use crate::commands::code_session::{list_past_sessions, SessionMeta};
