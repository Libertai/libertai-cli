...
- **CLI `/schedule` command** — REPL users can queue in-process
  ...
  counts, inspect one queued prompt with `/schedule show <id>`, and
  cancel them with `/schedule cancel <id>` or `/schedule clear`; `/cron`
  aliases match the desktop composer. Due prompts run
